...
export const STYLE_PROMPTS: Record<string, string> = {
  ...
  [RenderStyle.GOLDEN_HOUR_DRAMATIC]: "Dramatic Golden Hour Architectural Photography. Award-winning professional shot captured on a Pro grade Hasselblad medium format camera. The building is struck by intense, warm golden sunlight (low angle sun) against a dark, moody, storm-grey sky. High contrast lighting. The building glows in warm orange/gold tones while the background and shadows remain deep and cool. IMPORTANT: Ensure all architectural elements are visible and detailed, even in the darker shadow areas. Do not crush the blacks. Maintain the overall dramatic mood but ensure clarity. Generate realistic surrounding landscape and context as per the user's input, seamlessly integrated with the scene. 8k resolution, ultra-sharp focus. STRICTLY PRESERVE INPUT GEOMETRY. Ultimate realism.",
